...
func (r Resolver) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	if err != nil {
		logs.Log.Fatal("grpc client connect etcd err:", zap.Error(err))
	}
...
func (r Resolver) sync() error {
	...
	if err != nil {
		logs.Log.Error("grpc client get etcd failed", zap.Any("name", r.key), zap.Error(err))

		return err
	}

	logs.Log.Info("get  key", zap.Any("key", res.Kvs))

	r.srvAddrList = []resolver.Address{}
	for _, v := range res.Kvs {
		server, err := ParseValue(v.Value)
		if err != nil {
			logs.Log.Error("grpc client parse etcd value failed", zap.Any("name", r.key), zap.Error(err))
	...
	if len(r.srvAddrList) == 0 {
		logs.Log.Error("no services found")
	...
	if err != nil {
		logs.Log.Error("rpc client UpdateState failed", zap.Any("name", r.key), zap.Error(err))
...
func (r Resolver) watch() {
	...
	for {
		...
		case <-ticker.C:
			if err := r.sync(); err != nil {
				logs.Log.Error("watch sync failed", zap.Error(err))
			}
		}
	}
}
	...
	for _, ev := range events {
		...
		case clientv3.EventTypePut:
			...
			if !Exist(r.srvAddrList, addr) {
				...
				if err != nil {
					logs.Log.Error("grpc client update(EventTypePut) UpdateState failed", zap.Any("name", r.key), zap.Error(err))
				}
			}
		case clientv3.EventTypeDelete:
			...
			if err != nil {
				logs.Log.Error("grpc client update(EventTypeDelete) parse etcd value failed", zap.Any("name", r.key), zap.Error(err))
			}
			...
			if list, ok := Remove(r.srvAddrList, addr); ok {
				...
				if err != nil {
					logs.Log.Error("grpc client update(EventTypeDelete) UpdateState failed", zap.Any("name", r.key), zap.Error(err))
				}
			}
		}
	}
...
func (r Resolver) Close() {
	if r.etcdCli != nil {
		err := r.etcdCli.Close()
		if err != nil {
			logs.Log.Error("Resolver close etcd failed ", zap.Error(err))
		}
		logs.Log.Info("close etcd...")
	}
}
