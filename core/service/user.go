...
func (s *UserService) FindAndSaveUserByUid(ctx context.Context, uid string, info request.UserInfo) (*entity.User, error) {
	...
	if err != nil {
		logs.Log.Error("[UserService] FindAndSaveUserByUid  user err", zap.Error(err))
	...
	if user == nil {
		...
		user.Avatar = tool.Default(info.Avatar, "Common/head_icon_default")
		user.Nickname = tool.Default(info.Nickname, fmt.Sprintf("%s%s", "码神", uid))
		...
		if err != nil {
			logs.Log.Error("[UserService] FindAndSaveUserByUid insert user err:", zap.Error(err))
			return nil, err
		}
	}
...
func (s *UserService) FindUserByUid(ctx context.Context, uid string) (*entity.User, *msError.Error) {
	...
	user, err := s.userDao.FindUserByUid(ctx, uid)
	if err != nil {
		logs.Log.Error("[UserService] FindUserByUid  user err", zap.Error(err))
...
func (s *UserService) UpdateUserAddressByUid(uid string, req hall.UpdateUserAddressReq) error {
	...
	if err != nil {
		logs.Log.Error("userDao.UpdateUserAddressByUid err", zap.Error(err))
		return err
	}
	return nil
}
...
func (s *UserService) BindPhone(uid string, phone string) *msError.Error {
	...
	if err != nil {
		logs.Log.Error("FindAccountByPhone err", zap.Error(err))
	...
	if err != nil {
		logs.Log.Error("UpdatePhone err ", zap.Error(err))
...
func (s *UserService) GetUserData(phone string, uid string) (*entity.User, *msError.Error) {
	if uid != "" {
		...
		if err != nil {
			logs.Log.Error("FindUserByUid err ", zap.Error(err))
		...
		if err != nil {
			logs.Log.Error("FindUserByPhone err ", zap.Error(err))
...
func (s *UserService) UpdateUserRoomId(ctx context.Context, uid string, roomId string) error {
	...
	if err != nil {
		logs.Log.Error("UpdateUserRoomId err", zap.Error(err))
...
func (s *UserService) UpdateUserDataScoreInc(uid string, unionID int64, score int) *entity.User {
	...
	if err != nil {
		logs.Log.Error("FindAndUpdate err ", zap.Error(err))
...
func (s *UserService) SaveUserScoreChangeRecordList(arr []*entity.UserScoreChangeRecord) error {
	...
	if err != nil {
		logs.Log.Error("saveUserScoreChangeRecordList err ", zap.Error(err))
